...
import { UnifiedTabs } from '../common/UnifiedTabs';
import { SummaryContent } from '../common/SummaryContent';

export const TeacherInteractionResults: React.FC<ScreenProps> = ({ onNavigate, hasInteraction, onActivityFinished, questions = [], onReorderQuestions, selectedCourse, selectedActivity, onSetEditingInteraction, activeInteractionId, onToggleActiveInteraction, responseCounts, consultas = [], onUpdateConsultation }) => {
                        ...
                        {questions.map((q, idx) => (
                          ...
                          <Draggable key={q.id} draggableId={q.id.toString()} index={idx}>
                            {(provided) => (
                              ...
                              >
                                <div {...provided.dragHandleProps} className="text-[#c1c6d6] flex items-center justify-center -ml-2 w-6 hover:text-[#727785] cursor-grab active:cursor-grabbing">
                                  <span className="material-symbols-outlined text-[14px]">drag_indicator</span>
                                </div>
                                <div className="w-8 h-8 flex items-center justify-center bg-[#f2f3fd] rounded-full flex-shrink-0">
                                  <span className="material-symbols-outlined text-[#475e8c] text-[16px]">
                                ...
                                <div className="flex-grow min-w-0">
                                  <h3 className="font-manrope font-bold text-[#191c23] truncate text-[12px] leading-tight">{q.text}</h3>
                                  <div className="flex items-center gap-1.5 mt-0.5">
                                    <div className="flex items-center gap-1 px-1 py-0.5 bg-[#005bbf]/5 rounded shadow-sm">
                                      <span className="material-symbols-outlined text-[#005bbf] text-[10px]">poll</span>
                                      <span className="text-[8px] font-bold text-[#005bbf]">{responseCounts?.[q.id] || 0} rptas.</span>
                                    </div>
                                  </div>
                                </div>
                                <div className="flex items-center gap-1.5">
                                  <button 
                                    ...
                                    className="w-7 h-7 rounded-full flex items-center justify-center text-[#727785] hover:bg-[#f2f3fd] transition-colors border border-[#c1c6d6]/20"
                                    title="Editar"
                                  >
                                  ...
                                  <button 
                                    ...
                                    className={`flex items-center justify-center gap-1 h-7 w-[72px] rounded-lg font-bold text-[8px] uppercase tracking-wider transition-all active:scale-95 shadow-sm border ${
              ...
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                {resultsView === 'individual' ? (
                  <>
                    ...
                    {/* Visualizations */}
                    <div className="bg-[#f2f3fd] p-6 rounded-2xl relative">
                      {(() => {
                        const q = questions[selectedQuestion - 1];
                        if (!q) return null;
