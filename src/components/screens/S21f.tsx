...
export const S21f: React.FC<ScreenProps> = ({ onNavigate, onInteractionCreated, onInteractionUpdated, editingInteraction, onSetEditingInteraction, selectedCourse, selectedActivity, selectedInteractionType, activeInteractionId, responseCounts }) => {
  ...
  const isValid = questionText.trim() !== '' && (
    (isMultipleChoice && options.filter(o => o.trim() !== '').length >= 2 && correctOptions.length > 0) ||
    (isTrueFalse && correctOptions.length > 0) ||
    (isRanking && options.filter(o => o.trim() !== '').length >= 2) ||
    (isWordCloud || isFreeText) ||
    (!isMultipleChoice && !isTrueFalse && !isWordCloud && !isRanking && !isFreeText && (
  ...
  return (
    <div className="bg-[#f9f9ff] selection:bg-[#d8e2ff] selection:text-[#001a41] min-h-screen flex flex-col font-inter">
      <Header 
        ...
        onBack={handleBack} 
      />
      <Breadcrumb items={['Mis Cursos', selectedCourse?.name || 'Curso', selectedActivity?.name || 'Actividad', editingInteraction ? 'Editar pregunta' : 'Nueva pregunta']} />
