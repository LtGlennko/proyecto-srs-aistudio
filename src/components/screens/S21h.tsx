...
export const S21h: React.FC<ScreenProps> = ({ onNavigate, hasInteraction, onActivityFinished, questions = [] }) => {
  ...
  return (
    <div className="bg-[#f9f9ff] text-[#191c23] min-h-screen flex flex-col font-inter relative overflow-hidden">
      ...
      {/* S21i Modal - Centered Confirmation */}
      {showConfirm && (
        <>
          <div 
          ...
          <div className="absolute inset-0 flex items-center justify-center z-[2001] px-6">
            <div className="bg-white rounded-2xl p-6 w-full max-w-[320px] shadow-2xl animate-in zoom-in duration-200">
              <h2 className="font-manrope font-bold text-xl text-[#191c23] mb-3">Confirmación</h2>
              <p className="text-[#414754] text-sm leading-relaxed mb-8">
                La pregunta está incompleta. ¿Desea guardar las interacciones como borrador?
              </p>
              <div className="flex gap-3">
                <button 
                  onClick={() => setShowConfirm(false)}
                  className="flex-1 py-3 rounded-lg font-bold text-[#727785] bg-[#f2f3fd] hover:bg-[#e6e8f2] transition-colors"
                >
                  Volver
                </button>
                <button 
                  ...
                  className="flex-1 py-3 rounded-lg font-bold text-white bg-[#005bbf] hover:opacity-90 transition-all"
                >
                  Guardar
                </button>
              </div>
